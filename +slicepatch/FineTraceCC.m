%{
slicepatch.FineTraceCC (computed) # filter the traces in TraceCC to remove spikes and other noise. 
-> slicepatch.TraceCC
-----
...
classdef FineTraceCC < dj.Relvar & dj.AutoPopulate
    properties
        popRel = slicepatch.TraceCC
    end
	methods(Access=protected)

		function makeTuples(self, key)
            tuple = fetch(slicepatch.TraceCC & key, '*');
            key.time = tuple.time;
            key.led = tuple.led;
            key.dt = tuple.dt;
            cutfreq = 200;
